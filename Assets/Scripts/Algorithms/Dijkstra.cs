...
public class Dijkstra : MonoBehaviour
{
    ...
    private void Start()
    {
        //Use the function to store a list of nodes on the shortest path
        List<Node> _shortestPath = FindShortestPath(start, end);
        ...
        foreach (Node node in _shortestPath)
        {
            if (_previousNode != null)
            {
                Debug.DrawLine(node.transform.position + Vector3.up, _previousNode.transform.position + Vector3.up, Color.blue, 10f);
            }
    ...
    private bool DijkstraAlgorithm(Node start, Node end)
    {
        ...
        //For each GameObject in our list of nodes from the heirarchy try to retrieve the Node class and reset its values then add it to unexplored list
        ...
        while (_unexplored.Count > 0)
        {
            ...
            _unexplored.Sort((x, y) => x.NodeWeight.CompareTo(y.NodeWeight));
